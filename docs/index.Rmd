---
title: "The Impacts of Ocean Acidification"
date: "Winter 2022"
...
source("../source/visualization-1.R")
source("../source/summary_script.R")
source("../source/ph vs co2.R")
source("../source/ph by month.R")
```
...
The first dataset is collected over two decades made up of over 3,000 oceanographic profiles from 61 research ships. There are over 28,000 observations made in the dataset, each with multiple oceanic parameter variables recorded. 
...
```{r, echo=F}
kable(acidification_table)
```
<br>

#### Preliminary Look at Important Observations
In 2018, the average pH of our oceans was `r summary_info$recent_pH_level`. <br>
In 2004, the average pH of our oceans was `r summary_info$earliest_pH_level`. <br>
The decrease of `r summary_info$pH_level_difference` may not seem like much, but considering that the pH scale is logarithmic and many forms of life rely on a very tiny range of pH variance, this is a huge increase in acidity.
<br>

The most observations came from the Latitude and Longitude `r summary_info$most_sampled`, or roughly the middle of the gulf of mexico.<br>
`r summary_info$year_highest_fCO2` Was the highest measured CO2 fungacity (roughly, vapor pressure).
<br>
```{r, echo=FALSE, message=FALSE, results='hide'}
co2_by_ph
```
<br>
For this chart we decided to create a scatter plot to show the pH level (x axis) vs the average concentration of CO2 dissolved in the water. We calculated the average pH level and average concentration of CO2 for each month and plotted them on a graph. We were attempting to see if there is a correlation between the pH level and amount of dissolved carbon dioxide in ocean water. We are able to see a clear trend downward. When the average pH level is higher, the amount of dissolved carbon dioxide is lower, so the more CO2 that is absorbed into the ocean the more acidic the ocean becomes.
<br>
```{r, echo=FALSE, message=FALSE, results='hide'}
mean_ph_plot
```
<br>
We decided to create a line chart in an attempt to show the pH level change over the course of time. We thought a line chart was the best way to visualize this data. It is important to track the pH level as this may give us insight into how the ocean’s acidity is changing over time, and a chart showing the readings over time is the best way to visualize this data. We are able to see a clear trend downward in pH over time, meaning that the ocean samples become more acidic as time goes on.
<br>
```{r, echo=FALSE}
collection_map
```
<br>
In this map we show where each of our samples were taken. This gives good perspective to what kind of data we are analyzing, in that most of it is taken from areas close to shorelines. This is likely for two reasons, the first being that it is not practical to take measurements in the middle of the ocean, and that the data gathered 100 miles vs 2000 miles from shore is likely not radically different. Additionally, this map shows the defined range of each ocean so results may be analyzed by region. 
